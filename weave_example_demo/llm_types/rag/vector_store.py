...
from scipy.spatial.distance import cityblock, chebyshev, minkowski, hamming, jaccard


class VectorStore(weave.Object):

    embedding_model: Optional[Union[str,
                                    SentenceTransformersModel]] = "text-embedding-3-small"
    ...
    ranking_method: Literal["cosine", "euclidean", "manhattan",
                            "chebyshev", "minkowski", "hamming", "jaccard"] = "cosine"
    ...
    def __init__(
        ...
        super().__init__(embedding_model=embedding_model, articles=articles,
                         key=key, multiprocess_embeddings=multiprocess_embeddings)
    ...
    @weave.op()
    def set_embedding_model(self, embedding_model: Optional[Union[str, SentenceTransformersModel]] = "text-embedding-3-small"):
        self.embedding_model = embedding_model
        self.weave_init_embedding_model(embedding_model)

    @weave.op()
    def weave_init_embedding_model(self,
                                   embedding_model: Optional[Union[str,
                                                                   SentenceTransformersModel]] = "text-embedding-3-small"
                                   ):
        func_name = None
    ...
    def docs_to_embeddings(self, docs: Union[List[dict], List[str]]) -> List[dict]:
        ...
        def embed_doc(doc):
            ...
            response = self.embedding_function(
                input=text_to_embed, model=self.embedding_model).data[0]["embedding"]
            return {
                "embedding": response,
                "original_doc": doc
            }
        ...
        if self.multiprocess_embeddings:
            with concurrent.futures.ThreadPoolExecutor() as executor:
                ...
                for future in tqdm(concurrent.futures.as_completed(futures), total=len(futures), desc="Embedding documents"):
    ...
    @weave.op()
    def get_most_relevant_documents(self, query: str, n: int = 1, ranking_method: Optional[Literal["cosine", "euclidean", "manhattan", "chebyshev", "minkowski", "hamming", "jaccard"]] = None) -> list:
        if ranking_method:
    ...
    def _get_cosine_distances(self, query: str, n: int) -> list:
        query_embedding = self.embedding_function(
            input=query, model=self.embedding_model).data[0]["embedding"]
        cosine_similarities = cosine_similarity(
            [query_embedding], self.embeddings_matrix).flatten()
        top_n_indices = np.argsort(cosine_similarities)[-n:][::-1]
        return [{"document": self.article_embeddings[i]["original_doc"], "score": cosine_similarities[i]} for i in top_n_indices]
    ...
    def _get_euclidean_distances(self, query: str, n: int) -> list:
        ...
        return [{"document": self.article_embeddings[i]["original_doc"], "score": euclidean_distances[i]} for i in top_n_indices]

    @weave.op()
    def _get_manhattan_distances(self, query: str, n: int) -> list:
        query_embedding = self.embedding_function(
            input=query, model=self.embedding_model).data[0]["embedding"]
        distances = np.array([cityblock(query_embedding, emb)
                             for emb in self.embeddings_matrix])
        top_n_indices = np.argsort(distances)[:n]
        return [{"document": self.article_embeddings[i]["original_doc"], "score": distances[i]} for i in top_n_indices]

    @weave.op()
    def _get_chebyshev_distances(self, query: str, n: int) -> list:
        query_embedding = self.embedding_function(
            input=query, model=self.embedding_model).data[0]["embedding"]
        distances = np.array([chebyshev(query_embedding, emb)
                             for emb in self.embeddings_matrix])
        top_n_indices = np.argsort(distances)[:n]
        return [{"document": self.article_embeddings[i]["original_doc"], "score": distances[i]} for i in top_n_indices]
    ...
    def _get_minkowski_distances(self, query: str, n: int, p: int = 3) -> list:
        query_embedding = self.embedding_function(
            input=query, model=self.embedding_model).data[0]["embedding"]
        distances = np.array([minkowski(query_embedding, emb, p)
                             for emb in self.embeddings_matrix])
        top_n_indices = np.argsort(distances)[:n]
        return [{"document": self.article_embeddings[i]["original_doc"], "score": distances[i]} for i in top_n_indices]
    ...
    def _get_hamming_distances(self, query: str, n: int) -> list:
        query_embedding = self.embedding_function(
            input=query, model=self.embedding_model).data[0]["embedding"]
        distances = np.array([hamming(query_embedding, emb)
                             for emb in self.embeddings_matrix])
        top_n_indices = np.argsort(distances)[:n]
        return [{"document": self.article_embeddings[i]["original_doc"], "score": distances[i]} for i in top_n_indices]
    ...
    def _get_jaccard_distances(self, query: str, n: int) -> list:
        query_embedding = self.embedding_function(
            input=query, model=self.embedding_model).data[0]["embedding"]
        distances = np.array([jaccard(query_embedding, emb)
                             for emb in self.embeddings_matrix])
        top_n_indices = np.argsort(distances)[:n]
        return [{"document": self.article_embeddings[i]["original_doc"], "score": distances[i]} for i in top_n_indices]
