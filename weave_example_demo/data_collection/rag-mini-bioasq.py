...
def preprocess_data(qap, tc):
    ...
    qap_df['relevant_passage_ids'] = qap_df['relevant_passage_ids'].apply(
        ast.literal_eval)
    qap_df.rename(columns={'answer': 'ground_truth'},
                  inplace=True)  # Rename column here
    tc_df = tc.to_pandas()

    tc_df["passage"] = tc_df["passage"].str.strip()
    nan_passage_ids = tc_df[tc_df["passage"].isin(
        ["nan", "passage", "1."])]["id"].tolist()
...
def select_passage_ids(passage_id_counts, sample_frac=0.015, random_state=42):
    passage_id_df = pd.DataFrame(
        {'id': list(passage_id_counts.keys()), 'count': list(passage_id_counts.values())})
...
def filter_qap_df(qap_df, selected_passage_ids):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        qap_df_filtered['relevant_passage_ids'] = list(executor.map(lambda ids: filter_passage_ids(
            ids, selected_passage_ids), qap_df_filtered['relevant_passage_ids']))
    qap_df_filtered = qap_df_filtered[qap_df_filtered['relevant_passage_ids'].apply(
        bool)]
...
def create_filtered_bioasq_datasets(args):
    ...
    return {"text_corpus": tc_dataset_filtered, "qap_train": qap_train_dataset_filtered, "qap_test": qap_test_dataset_filtered}
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sample_frac', type=float,
                        default=0.015, help='Fraction of samples to select')
