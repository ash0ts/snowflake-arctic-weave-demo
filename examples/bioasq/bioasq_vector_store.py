...
def get_relevant_documents(vector_store, query, n, ranking_method=None):
    if ranking_method:
        return vector_store.get_most_relevant_documents(
            query=query, n=n, ranking_method=ranking_method
        )
...
def create_bioasq_vector_store(args):
    ...
    vector_store = VectorStore(
        articles=dataset_ref,
        key=args["key"],
        embedding_model=embedding_model,
        multiprocess_embeddings=args["multiprocess_embeddings"],
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--store_name",
        type=str,
        default="BioASQVectorStore",
        help="Name to publish the vector store under",
    )
    parser.add_argument(
        "--alias", type=str, default="latest", help="Alias for the dataset reference"
    )
    parser.add_argument(
        "--dataset_name",
        type=str,
        default="TextCorpusFiltered",
        help="Name of the dataset to publish the vector store under",
    )
    parser.add_argument(
        "--multiprocess_embeddings",
        type=bool,
        default=True,
        help="Whether to use multiprocess embeddings",
    )
    parser.add_argument(
        "--key", type=str, default="passage", help="Key to use for the vector store"
    )
    parser.add_argument(
